...
def test_aromatic_interaction_working_query(path_runner):
    assert call(
        f'python {path_runner} --ai 1rcy'.split(),
        stdout=DEVNULL
    ) == EXIT_SUCCESS
...
def test_bridging_interaction_working_query(path_runner):
    assert call(
        f'python {path_runner} --bi 1rcy'.split(),
        stdout=DEVNULL
    ) == EXIT_SUCCESS
...
def test_bridging_interaction_working_query_vertices_3(path_runner):
    assert call(
        f'python {path_runner} --bi 1rcy --vertices 3'.split(),
        stdout=DEVNULL
    ) == EXIT_SUCCESS
...
def test_bridging_interaction_working_query_vertices_2(path_runner):
    assert call(
        f'python {path_runner} --bi 1rcy --vertices 2'.split(),
        stdout=DEVNULL
...
def test_invalid_cutoff_distance(path_runner):
    assert call(
        f'python {path_runner} --ai 1rcy --cutoff_distance 0.00'.split()
    ) == errors.ErrorCodes.InvalidCutoffsError


def test_invalid_cutoff_distance_stringified(path_runner):
    assert call(
        f'python {path_runner} --ai 1rcy --cutoff_distance foo'.split(),
        stderr=DEVNULL
