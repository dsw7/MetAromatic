...
class TestCrossProductMethod:
    ...
    def test_cp_vector_a(self) -> None:
        assert array_equal(
            around(self.frame_cp.get_vector_a(), decimals=2), [-1.0, 0.0, -1.41]
        )

    def test_cp_vector_g(self) -> None:
        assert array_equal(
            around(self.frame_cp.get_vector_g(), decimals=2), [-1.0, 0.0, 1.41]
        )
...
class TestRodriguesMethod:
    ...
    def test_rm_vector_a(self) -> None:
        assert array_equal(
            around(self.frame_rm.get_vector_a(), decimals=1), [-0.5, 0.0, -0.5]
        )

    def test_rm_vector_g(self) -> None:
        assert array_equal(
            around(self.frame_rm.get_vector_g(), decimals=1), [-0.5, 0.0, 0.5]
        )
